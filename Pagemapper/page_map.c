...
static struct dentry *pagetables_dir=NULL;
...
static int __init pagetables_init(void)
{
	...
	//filep = (struct dentry *)
	debugfs_create_x64("vaddr", 0600, pagetables_dir,&vaddr);
	if (IS_ERR_OR_NULL(filep))
		goto error;

	//filep = 
	debugfs_create_u32("pid", 0600, pagetables_dir,
				&target_pid_nr);
	if (IS_ERR_OR_NULL(filep))
		goto error;
